...
private:
    static BenchmarkResults run_single_test(HashMapBase* hashmap, int num_threads, int num_operations) {
        ...
        std::atomic<long long> insert_retries{0}, find_retries{0};
        ...
        results.avg_retries = (insert_retries + find_retries) / (2.0 * total_ops);
 ...
 static void print_summary(const TestConfig configs[],
    ...
    double total_time_base = baseline.insert_time_ms + baseline.find_time_ms;
    ...
    for (size_t impl = 0; impl < 4; ++impl) {
        ...
        for (size_t config = 0; config < all_results.size(); ++config) {
            ...
            double total_time = result.insert_time_ms + result.find_time_ms;
            double speedup = total_time_base / total_time;  // Fixed speedup calculation
...
public:
    static void run_benchmark_suite() {
        ...
        const TestConfig configs[] = {
            {"Low Contention", 4, 1000000},
            {"Medium Contention", 8, 500000},
            {"High Contention", 16, 250000},
            {"Very High Contention", 32, 125000},
            {"Very High Contention", 64, 62500}
        };
        ...
        for (size_t i = 0; i < config_count; ++i) {
            ...
            }
          {
              ...
              {
                LibCuckooHashMapWrapper HashTable1;
                config_results.push_back(run_single_test(&HashTable1, 
                                                         config.num_threads, config.num_operations));
            }
